    ...
    if let Some(ref opts) = options {
        if session.exists(&remote_path)? {
            match opts.overwrite.as_str() {
                ...
                "overwrite-older" => {
                    // Compare local mtime vs remote mtime; skip if remote is newer or equal
    ...
    if let Some(ref opts) = options {
        ...
        if local_exists {
            match opts.overwrite.as_str() {
                ...
                "overwrite-older" => {
                    // Compare remote mtime vs local mtime; skip if local is newer or equal
                    let local_mtime = std::fs::metadata(&local_path)
                        .ok()
                        .and_then(|m| m.modified().ok())
                        .and_then(|t| t.duration_since(std::time::UNIX_EPOCH).ok())
...
pub fn cancel_transfer(
    ...
    Ok(state.cancel(&transfer_id))
}

#[tauri::command]
pub fn ftp_disconnect(state: State<'_, FtpState>, hosting_id: String) -> Result<(), String> {
    let mut sessions = state.sessions.lock().map_err(|e| e.to_string())?;
